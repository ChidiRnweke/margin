...
export interface IRecurrenceService {
  upsertSeries(userId: string, input: UpsertSeriesInput, expectedVersionOrNone?: number): Promise<unknown>;
  pauseOrResumeSeries(userId: string, seriesId: string, paused: boolean, expectedVersion: number): Promise<unknown>;
  skipOrMoveNextOccurrence(userId: string, seriesId: string, input: SkipOrMoveInput, expectedVersion: number): Promise<unknown>;

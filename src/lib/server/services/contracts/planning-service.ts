...
export interface IPlanningService {
  ...
  regenerateConfirmedPlan(userId: string, cycleId: string, expectedVersion: number): Promise<unknown>;
  editPlan(userId: string, cycleId: string, input: { edits: AllocationEdit[] }, expectedVersion: number): Promise<PlanningRevisionSnapshot>;
  replanActiveCycles(now: Date): Promise<ReplanSummary>;
  queryCycles(userId: string, query: PlanningCycleQuery): Promise<{ items: unknown[]; nextCursor?: string }>;
}

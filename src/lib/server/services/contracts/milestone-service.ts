...
export interface IMilestoneService {
  ...
  updateMilestone(userId: string, milestoneId: string, input: UpdateMilestoneInput, expectedVersion: number): Promise<unknown>;
  ...
  queryMilestones(userId: string, query: MilestoneQuery): Promise<{ items: unknown[]; nextCursor?: string }>;
}

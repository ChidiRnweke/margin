...
:root {
	...
	--gradient-aurora: linear-gradient(135deg, hsl(var(--accent) / 0.25), hsl(var(--accent-2) / 0.2));
	--radius: 16px;
	--shadow-1: 0 12px 30px hsl(0 0% 0% / 0.1);
}
```
...
- Use gradients as a _directional light_ (top-left → bottom-right).
...
<section class="relative overflow-hidden rounded-[24px] p-8">
	...
	<div class="relative rounded-[16px] bg-[hsl(var(--surface))] p-6 shadow-[var(--shadow-1)]">
		...
		<button class="mt-4 rounded-[9999px] bg-[hsl(var(--accent))] px-5 py-2 font-semibold text-black"
			>Try it</button
		>
	</div>
</section>
...
```svelte
<span
	class="inline-flex items-center rounded-[9999px] border border-black/10 bg-gradient-to-r from-cyan-500/30
             to-violet-500/30 px-3 py-1 text-xs font-semibold"
>
	New
</span>
```

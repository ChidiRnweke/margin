...
:root {
	...
	--radius: 16px;
	--shadow-1: 0 6px 18px hsl(0 0% 0% / 0.1);
}
```
...
- Keep core components consistent; illustrations sit _around_ them.
...
>
	<div class="mx-auto grid h-28 w-28 place-items-center rounded-[24px] bg-[hsl(var(--surface-2))]">
		<!-- Replace with your SVG illustration -->
		<span class="text-3xl">🥕</span>
	</div>
	<h3 class="mt-5 text-xl font-semibold">Your pantry is empty</h3>
	<p class="mx-auto mt-2 max-w-sm text-sm text-[hsl(var(--text)/0.75)]">
		Add a few staples and we’ll start suggesting meals you can make.
	</p>
	<button
		class="mt-5 rounded-[9999px] bg-[hsl(var(--accent))] px-6 py-2.5 font-semibold text-[hsl(var(--text))]"
	>
		Add staples
	</button>
</div>
...
```svelte
<aside
	class="flex gap-3 rounded-[var(--radius)] border border-[hsl(var(--border))]
             bg-[hsl(var(--surface-2))] p-4"
>
	<div class="grid h-10 w-10 place-items-center rounded-[12px] bg-white">✨</div>
	<div>
		<p class="text-sm font-semibold">Tip</p>
		<p class="text-sm text-[hsl(var(--text)/0.75)]">
			Mark items as “Use soon” to get better suggestions.
		</p>
	</div>
</aside>
```

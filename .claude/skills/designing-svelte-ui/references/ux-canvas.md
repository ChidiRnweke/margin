...
<div
	class="relative h-screen w-full cursor-grab overflow-hidden bg-background active:cursor-grabbing"
	onwheel={handleWheel}
	role="application"
...
>
	...
	<div
		class="absolute bottom-6 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-full border border-border bg-background/80 px-2 py-2 shadow-lg backdrop-blur-sm"
	>
		<Button
			variant="ghost"
			size="icon"
			class="rounded-full"
			onclick={() => (pos.scale += 0.1)}
			title="Zoom In"
		>
			<Plus class="size-4" />
		</Button>
